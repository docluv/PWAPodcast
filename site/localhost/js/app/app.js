( function () {
    ...
    function bindEpisodeFavorite() {

        var favBtn = document.querySelector( ".episode-favorite" );

        if ( favBtn ) {

            favBtn.addEventListener( "click", toggleEpisodeFavorite );
        }

    }

    function toggleEpisodeFavorite( evt ) {

        evt.preventDefault();

        var target = evt.currentTarget,
            src = target.getAttribute( "audio-src" );

        var url = new URL( src );

        localforage.getItem( url.pathname )
            .then( function ( obj ) {
                ...
                if ( obj ) {
                    ...
                    target.classList.add( "episode-favorited" );

                } else {

                    return fetchAndStore( url.pathname );
    ...
    function fetchAndStore( src ) {
        ...
        return fetch( src, {
            ...
            .then( function ( response ) {
                ...
                if ( [ 0, 200 ].indexOf( response.status ) > -1 ) {

                    return localforage.setItem( url.pathname, response.blob() )
                        .then( function ( obj ) {
